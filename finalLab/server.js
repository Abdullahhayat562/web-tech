...
const mongoUri =
  process.env.MONGO_URI || "mongodb://127.0.0.1:27017/lab_assignment_4";
const SESSION_SECRET =
  process.env.SESSION_SECRET || "lab-assignment-4-session-secret";
const JWT_SECRET =
  process.env.JWT_SECRET ||
  "replace-this-development-jwt-secret-with-a-strong-env-value";
const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || "1h";
const DEFAULT_ADMIN_NAME = process.env.ADMIN_NAME || "Store Admin";
const DEFAULT_ADMIN_EMAIL = process.env.ADMIN_EMAIL || "admin@mvmt.local";
...
function buildProductQuery(filters) {
  ...
  const parsedMinPrice = filters.minPrice ? Number(filters.minPrice) : "";
  const parsedMaxPrice = filters.maxPrice ? Number(filters.maxPrice) : "";
...
app.get("/api/v1/products", async function (req, res) {
  try {
    ...
    const limit = Math.min(
      Math.max(parseInt(req.query.limit, 10) || PRODUCTS_PER_PAGE, 1),
      50
    );
    const { query, filters } = buildProductQuery(req.query);
...
app.get("/api/v1/user/profile", verifyToken, async function (req, res) {
  try {
...
app.post("/api/v1/orders", verifyToken, async function (req, res) {
  ...
  try {
    ...
    if (hasInvalidProductId) {
      return res.status(400).json({
        success: false,
        message: "Each order item must include a valid productId.",
      });
    }
  ...
  try {
    ...
    return res.render("admin/dashboard", {
      ...
      stats: {
        ...
        onSaleProducts: products.filter((product) => product.isOnSale).length,
      },
    });
  } catch (error) {
...
app.get("/onsale-products", expressLayouts, async function (req, res) {
  try {
    ...
    return res.render("onsale", {
      activePage: "onsale",
      pageTitle: "On Sale Products - MVMT Catalog",
      products,
      includeJquery: true,
      pageScripts: ["/js/onsale.js"],
    });
  } catch (error) {
    console.error("On-sale products route error:", error.message);
    return res.status(500).send("Unable to load on-sale products.");
  }
});
